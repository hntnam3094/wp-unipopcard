...
add_action('init', 'create_custom_post_type_short_blog');

function create_custom_post_type_short_blog()
{
    /*
     * Biến $label để chứa các text liên quan đến tên hiển thị của Post Type trong Admin
     */
    $label = array(
        'name' => 'Short blog', //Tên post type dạng số nhiều
    );
...
//            'thumbnail'
    ...
    register_post_type('short_blog', $args); //Tạo post type với slug tên là craftcollection và các tham số trong biến $args ở trên

//    register_taxonomy('category', array('short_blog', 'craft'), array("hierarchical" => false, "label" => "Chuyên mục"));
//    register_taxonomy('post_tag', array('craft'), array("hierarchical" => false));
}
